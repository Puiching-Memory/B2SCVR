...
# from model.modules.feat_refine import RefineModule
...
class InpaintGenerator(BaseNetwork):
    ...
    def forward(self, masked_frames, corr_content, num_local_frames):
        ...
        # local_feat = self.feat_refine_module(enc_feat.view(b, t, c, h, w)[:, :l_t, ...], 
        #                                       enc_corr_feat.view(b, t, c, h, w)[:, :l_t, ...])

        # feature propagation
        local_feat = self.feat_prop_module(local_feat, pred_flows[0],
                                           pred_flows[1])
        # enc_feat = torch.cat((local_feat, ref_feat), dim=1)
        enc_feat = torch.cat((local_feat, 
                              enc_corr_feat.view(b, t, c, h, w)[:, l_t:, ...]), 
                              dim=1)
    ...
    def train_forward(self, masked_frames, corr_content, num_local_frames, gt_frames):
        l_t = num_local_frames
        b, t, ori_c, ori_h, ori_w = masked_frames.size()
        # print("input masked_frames.size(): ", masked_frames.size())
        ...
        enc_feat = self.encoder(masked_frames.view(b * t, ori_c, ori_h, ori_w))
        enc_corr_feat = self.encoder(corr_content.view(b * t, ori_c, ori_h, ori_w))
        # enc_gt_feat = self.encoder(gt_frames.view(b * t, ori_c, ori_h, ori_w))
        _, c, h, w = enc_feat.size()
        fold_output_size = (h, w)
        ...
        local_feat = self.feat_refine_module.train_forward(enc_feat.view(b, t, c, h, w)[:, :l_t, ...], 
                                              enc_corr_feat.view(b, t, c, h, w)[:, :l_t, ...])
                                            #   enc_gt_feat.view(b, t, c, h, w))  # memory efficient v1p
